...
CREATE TABLE objects (
    ...
    updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    deleted_at DATETIME(6) NOT NULL DEFAULT '9999-12-31 23:59:59',
    UNIQUE unq_data_id_name (data_id, name, deleted_at),
    UNIQUE unq_bucket_id_name_version (bucket_id, name, version, deleted_at),
    CONSTRAINT object_region_fk FOREIGN KEY (region) REFERENCES regions (name) ON UPDATE CASCADE,
    CONSTRAINT object_data_id_fk FOREIGN KEY (data_id) REFERENCES data (id) ON UPDATE CASCADE,
